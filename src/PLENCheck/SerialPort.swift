...
class SerialPort: NSObject, ConnectProcess, ORSSerialPortDelegate {
    ...
    convenience init(delegate: PlenCommDelegate, devicePath: String!) {
        self.init()
    ...
    func SendCommand() {
        ...
        dispatch_async(dispatch_get_main_queue(), {
            self.delegate.MessageFromCommProcess("header written.")
        })
        NSThread.sleepForTimeInterval(0.1)
        ...
        do {
            ...
            if isPlenConnected == true {
                // 画面表示はメインスレッドと同期して行う
                dispatch_async(dispatch_get_main_queue(), {
                    self.delegate.MessageFromCommProcess("frame written.  : \(++sendCnt)/\(frameCnt)")
                })
            }
    ...
    func PlenConnect() {
        ...
        delegate.MessageFromCommProcess("Connected")
    }
    
    func PlenDisconnect() {
        if serialPort != nil {
            serialPort!.close()
            delegate.MessageFromCommProcess("Disconnected.")
